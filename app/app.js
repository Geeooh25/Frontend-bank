 // Complete functionality for all toggles including forgot password
    ...
    document.querySelectorAll('.quick-action').forEach(action => {
      ...
      document.addEventListener('DOMContentLoaded', function() {
        ...
        const pinInstructions = document.querySelector('.pin-instructions');
        ...
        function setupCardInteractions() {
          ...
          viewDetailsButtons.forEach(button => {
            button.addEventListener('click', function(e) {
              ...
              const cardId = cardElement.classList[1]; // Get card type from class
              const detailsPanel = document.getElementById(`${cardId}-details`);
          ...
          cardActionButtons.forEach(button => {
            if (!button.classList.contains('view-details-btn')) {
              button.addEventListener('click', function(e) {
                ...
                switch(action) {
                  case 'Freeze Card':
                    showFreezeCardDialog(cardNumber);
                    break;
                  case 'Copy Card Number':
                    copyCardNumber(cardElement);
                    break;
                }
              });
            }
        ...
        function setupPINManagement() {
          ...
          setPinAmex.addEventListener('click', () => openPinModal('set', 'American Express •••• 3456'));
          setPinDiscover.addEventListener('click', () => openPinModal('set', 'Discover •••• 7890'));
          
          // Change PIN buttons for cards with PIN
          changePinVisa.addEventListener('click', () => openPinModal('change', 'Visa Debit •••• 4012'));
          changePinMastercard.addEventListener('click', () => openPinModal('change', 'MasterCard •••• 5678'));
          changePinVirtual.addEventListener('click', () => openPinModal('change', 'Virtual Card •••• 9012'));
        }
        ...
        function checkPINCompletion() {
          const pinInputs = document.querySelectorAll('.pin-input:not(.confirm)');
          ...
          if (confirmPinGroup.style.display !== 'none') {
        ...
        function openPinModal(mode, cardName) {
          ...
          if (mode === 'set') {
            ...
            pinInstructions.textContent = 'Enter your 4-digit PIN';
            confirmPinGroup.style.display = 'none';
          } else {
            pinModalTitle.textContent = 'Change PIN';
            pinInstructions.textContent = 'Enter your current 4-digit PIN';
        ...
        function handlePinChange() {
          ...
          setTimeout(() => {
            ...
            // Update PIN status in the UI
            // This would typically update the database and refresh the UI
          }, 1000);
        }
        ...
        function showFreezeCardDialog(cardNumber) {
          // This would typically open a confirmation dialog
          console.log(`Freezing card: ${cardNumber}`);
        }

        function copyCardNumber(cardElement) {
          const cardNumber = '9012345678901234'; // This would come from your data
          navigator.clipboard.writeText(cardNumber).then(() => {
            // Show feedback
            const button = cardElement.querySelector('[title="Copy Card Number"]');
            const originalHTML = button.innerHTML;
            button.innerHTML = '<i class="fas fa-check"></i>';
            button.style.color = '#0a9c4a';
